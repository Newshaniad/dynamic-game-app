  ...
  st.markdown("""
  Game Description
  You will be matched with another player and play a 2-period dynamic game. In each period, you
  simultaneously choose an action.
  ...
  def create_comprehensive_pdf():
      ...
      def create_enhanced_chart(choices, labels, title, filename, player_type):
          if len(choices) > 0:
              import pandas as pd
              counts = pd.Series(choices).value_counts(normalize=True).reindex(labels, fill_value=0) *       
  100
              ...
              # Color scheme
              colors_p1 = ['#1f77b4', '#ff7f0e'] if player_type == "P1" else ['#1f77b4', '#ff7f0e',
  '#2ca02c']
      ...
      chart_files = []
      if p1_choices_r1:
          chart_files.append(create_enhanced_chart(p1_choices_r1, ["A", "B"], "Player 1 Choices (Period      
  1)", "p1_r1.png", "P1"))
      if p2_choices_r1:
          chart_files.append(create_enhanced_chart(p2_choices_r1, ["X", "Y", "Z"], "Player 2 Choices
  (Period 1)", "p2_r1.png", "P2"))
      if p1_choices_r2:
          chart_files.append(create_enhanced_chart(p1_choices_r2, ["A", "B"], "Player 1 Choices (Period      
  2)", "p1_r2.png", "P1"))
      if p2_choices_r2:
          chart_files.append(create_enhanced_chart(p2_choices_r2, ["X", "Y", "Z"], "Player 2 Choices
  (Period 2)", "p2_r2.png", "P2"))
  ...
  download="complete_game_results.pdf">Click here to download Complete Game Report</a>'
          ...
          with st.spinner("⏳ Waiting for the other player to submit their action..."):
              ...
              for _ in range(max_wait):
                  ...
                  if submitted and "Player 1" in submitted and "Player 2" in submitted:
                      ...
                      st.success(f"🎯 Period 1 Outcome: P1 = {action1}, P2 = {action2} → Payoffs =
  {payoff}")
                  ...
                  st.warning("⌛ The other player hasn't submitted yet. Please wait a bit more and
  refresh.")
          ...
          if st.session_state.get("go_to_period2", False):
              ...
              if period1_data and "Player 1" in period1_data and "Player 2" in period1_data:
                  ...
                  st.info(f"📢 In Period 1: P1 = {action1}, P2 = {action2} → Payoffs =
  {period1_payoff}")
              ...
              if existing_action2:
                  ...
                  if role == "Player 1":
                      ...
                      choice2 = st.radio("Choose your Period 2 action:", ["X", "Y", "Z"],
  key="p2_period2")
              ...
              with st.spinner("⏳ Waiting for the other player to submit their action in Period 2..."):      
                  ...
                  for _ in range(max_wait):
                      ...
                      if submitted2 and "Player 1" in submitted2 and "Player 2" in submitted2:
                          ...
                          st.success(f"🎯 Period 2 Outcome: P1 = {action1_2}, P2 = {action2_2} → Payoffs     
   = {payoff2}")
                          st.balloons()
                          ...
                          if expected_players > 0 and completed_check >= expected_players:
                              st.success("🎉 All players have finished! Results are now available
  below.")
                      ...
                      st.warning("⌛ The other player hasn't submitted their Period 2 action yet. Please     
   wait and refresh.")
                      st.balloons()
                      st.markdown("✅ Game Complete! Thanks for playing.")
  ...
  if expected_players > 0 and completed_players >= expected_players and not
  st.session_state.get("all_games_complete", False):
      time.sleep(3)
      st.rerun()
  ...
  if expected_players > 0 and completed_players >= expected_players:
      ...
      with col2:
          plot_enhanced_percentage_bar(p2_choices_r1, ["X", "Y", "Z"], "Player 2 Choices (Period 1)",        
  "P2")
      ...
      with col4:
          plot_enhanced_percentage_bar(p2_choices_r2, ["X", "Y", "Z"], "Player 2 Choices (Period 2)",        
  "P2")

  elif expected_players > 0:
      st.info(f"⏳ Waiting for all participants to finish... ({completed_players}/{expected_players}
  players completed)")
  else:
  ...
  if st.button("🔄 Check for Updated Results"):
      ...
      if fresh_expected_players > 0 and fresh_completed_players >= fresh_expected_players:
          ...
          st.info(f"⏳ Still waiting... ({fresh_completed_players}/{fresh_expected_players} players
  completed)")
      else:
